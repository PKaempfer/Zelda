...
//			while(counterbread){
//				if(counterbread->dest && counterbread->dest->ID == i && pathlength == counterbread->dest->len && bread != counterbread){
////					if(verbose)
//						printf("Flag reverse path: %i from %i\n",bread->dest->ID,i);
//					counterbread->dest->flag = 1;
//					counterbread->dest->pathID = pathsNum;
//					break;
//				}
...
// * @param side
// */
//int connectPathsDFS(struct myovlList* G, struct readFiles* files, struct read* reads, int pathR2, int oldjunctionID, int newjunctionID, int side, int rID){
//
//	// if path found return 1 and stop all further recursive function calls
//	printf("Call with p: %i -> (path: %i , dist: %i) junction: %i side: %i\n",p,pathstack[(p-1)*PE],pathstack[(p-1)*PE+1],newjunctionID,side);
//	if(pathstack[(p-1)*PE] == pathR2){
//		int dist;
//		struct pc_anno* pc_anno = (struct pc_anno*)reads[rID].annotation;
//		if(paths[pathR2].leftJunction == oldjunctionID){
//			printf("Junction: %i dist: %i\n",oldjunctionID,pc_anno->lJunctionDist);
//			dist = pathstack[(p-2)*PE+1] + pc_anno->lJunctionDist;
//		}
//		else if(paths[pathR2].rightJunction == oldjunctionID){
//			printf("Junction: %i dist: %i\n",oldjunctionID,pc_anno->rJunctionDist);
...
//			printf("SUCCSESS -> CONCORDANCE ;-) \n");
//			return dist;
//		}
//		else{
//			printf("SUCCSESS -> But DISCORDANCE :-( \n");
//			return 0;
//		}
//	}
//	else{
...
//						int dist = connectPathsDFS(G,files,reads,pathR2,newjunctionID,bread->dest->ID,newSide ? 0 : 1, rID);
//						if (dist) return dist;
//						p--;
//					}
//					else{
//						printf("----> Distance (%i) is already to high. No further depth search\n",paths[newPath].len + pathstack[(p-1)*PE+1]);
//					}
//				}
//				else{
//					printf("Case should never happens: Abort\n");
//					exit(1);
//				}
//			}
//			bread = bread->next;
...
//void connectPathsPairwise(){
//	int verbose = 0;
//	printf("Verbose: %i\n",verbose);
//	int k,l;
//	for(k=1;k<p;k++){ // outer loop represents the path ends
//		for(l=0;l<k;l++){  // inner loop represents the path starts
//			// Connection in both directions l -> k, k -> l
//			// l -> k
//			if(verbose) printf("\t -> %i -> %i :path: %i -> %i (depth: %i) junction: %i\n",l,k,pathstack[l*PE],pathstack[k*PE],k-l,pathstack[l*PE+2]);
//			if(pathstack[l*PE+2] == paths[pathstack[l*PE]].leftJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[pathstack[l*PE]].leftPath;
//				while(pathEdge){
//					if(pathEdge->ID == pathstack[k*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
//						break;
//					}
//					pathEdge = pathEdge->next;
//				}
//				if(!pathEdge){
//					// Path not existent, search correct position (???) and create
//					pathEdge = (struct pathEdge*)malloc(sizeof(struct pathEdge));
//					pathEdge->ID = pathstack[k*PE];
//					pathEdge->targetJunction = pathstack[k*PE+2];
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					pathEdge->next = paths[pathstack[l*PE]].leftPath;
//					paths[pathstack[l*PE]].leftPath = pathEdge;
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",l,k,pathstack[l*PE],pathstack[k*PE],k-l,pathstack[l*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else if(pathstack[l*PE+2] == paths[pathstack[l*PE]].rightJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[pathstack[l*PE]].rightPath;
//				while(pathEdge){
//					if(pathEdge->ID == pathstack[k*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
...
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					pathEdge->next = paths[pathstack[l*PE]].rightPath;
//					paths[pathstack[l*PE]].rightPath = pathEdge;
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",l,k,pathstack[l*PE],pathstack[k*PE],k-l,pathstack[l*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else{
//				printf("Both junctions are not registered as end junctions of this path\nAbort\n");
//				exit(1);
//			}
//			// k -> l
//			if(verbose) printf("\t -> %i -> %i :path: %i -> %i (depth: %i) junction: %i\n",k,l,pathstack[k*PE],pathstack[l*PE],k-l,pathstack[(k-1)*PE+2]);
//			if(pathstack[(k-1)*PE+2] == paths[pathstack[k*PE]].leftJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[pathstack[k*PE]].leftPath;
//				while(pathEdge){
//					if(pathEdge->ID == pathstack[l*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
//						break;
//					}
...
//					// Path not existent, search correct position (???) and create
//					pathEdge = (struct pathEdge*)malloc(sizeof(struct pathEdge));
//					pathEdge->ID = pathstack[l*PE];
//					if(paths[pathstack[l*PE]].leftJunction == pathstack[l*PE+2]) pathEdge->targetJunction = paths[pathstack[l*PE]].rightJunction;
//					else pathEdge->targetJunction = paths[pathstack[l*PE]].leftJunction;
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					pathEdge->next = paths[pathstack[k*PE]].leftPath;
//					paths[pathstack[k*PE]].leftPath = pathEdge;
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",k,l,pathstack[k*PE],pathstack[l*PE],k-l,pathstack[(k-1)*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else if(pathstack[(k-1)*PE+2] == paths[pathstack[k*PE]].rightJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[pathstack[k*PE]].rightPath;
//				while(pathEdge){
//					if(pathEdge->ID == pathstack[l*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
...
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					pathEdge->next = paths[pathstack[k*PE]].rightPath;
//					paths[pathstack[k*PE]].rightPath = pathEdge;
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",k,l,pathstack[k*PE],pathstack[l*PE],k-l,pathstack[(k-1)*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else{
...
//void connectPathsPairwise2(){
//	int verbose = 0;
//	int k,l;
//	for(k=1;k<finp;k++){ // outer loop represents the path ends
//		for(l=0;l<k;l++){  // inner loop represents the path starts
//			// Connection in both directions l -> k, k -> l
//			// l -> k
//			// TODO Add new connection the the beginning
//			if(verbose) printf("\t -> %i -> %i :path: %i -> %i (depth: %i) junction: %i\n",l,k,finpathStack[l*PE],finpathStack[k*PE],k-l,finpathStack[l*PE+2]);
//			if(finpathStack[l*PE+2] == paths[finpathStack[l*PE]].leftJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[finpathStack[l*PE]].leftPath;
//				while(pathEdge){
//					if(pathEdge->ID == finpathStack[k*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
//						break;
//					}
...
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					pathEdge->next = paths[finpathStack[l*PE]].leftPath;
//					paths[finpathStack[l*PE]].leftPath = pathEdge;
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",l,k,finpathStack[l*PE],finpathStack[k*PE],k-l,finpathStack[l*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else if(finpathStack[l*PE+2] == paths[finpathStack[l*PE]].rightJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[finpathStack[l*PE]].rightPath;
//				while(pathEdge){
//					if(pathEdge->ID == finpathStack[k*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
...
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					pathEdge->next = paths[finpathStack[l*PE]].rightPath;
//					paths[finpathStack[l*PE]].rightPath = pathEdge;
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",l,k,finpathStack[l*PE],finpathStack[k*PE],k-l,finpathStack[l*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else{
//				printf("1 Both junctions are not registered as end junctions of this path\nAbort\n");
//				exit(1);
//			}
//			// k -> l
//			// TODO: Add new connection to the end: Does not work, it depends on the side.
//			if(verbose) printf("\t -> %i -> %i :path: %i -> %i (depth: %i) junction: %i\n",k,l,finpathStack[k*PE],finpathStack[l*PE],k-l,finpathStack[(k-1)*PE+2]);
//			if(finpathStack[(k-1)*PE+2] == paths[finpathStack[k*PE]].leftJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[finpathStack[k*PE]].leftPath;
//				struct pathEdge* lastpathEdge;
//				while(pathEdge){
//					if(pathEdge->ID == finpathStack[l*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
//						pathEdge->counter++;
//						break;
//					}
//					if(!pathEdge->next) lastpathEdge = pathEdge;
//					pathEdge = pathEdge->next;
//				}
//				if(!pathEdge){
//					// Path not existent, search correct position (???) and create
//					pathEdge = (struct pathEdge*)malloc(sizeof(struct pathEdge));
//					pathEdge->ID = finpathStack[l*PE];
//					if(paths[finpathStack[l*PE]].leftJunction == finpathStack[l*PE+2]) pathEdge->targetJunction = paths[finpathStack[l*PE]].rightJunction;
//					else pathEdge->targetJunction = paths[finpathStack[l*PE]].leftJunction;
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					if(!paths[finpathStack[k*PE]].leftPath){
//						pathEdge->next = paths[finpathStack[k*PE]].leftPath;
//						paths[finpathStack[k*PE]].leftPath = pathEdge;
//					}
//					else{
//						pathEdge->next = NULL;
//						lastpathEdge->next = pathEdge;
//					}
//
//					if(verbose) printf("NEW PATH: \t -> %i -> %i :path: %i -> %i (depth: %i) over junction: %i / target junction: %i\n",k,l,finpathStack[k*PE],finpathStack[l*PE],k-l,finpathStack[(k-1)*PE+2],pathEdge->targetJunction);
//				}
//			}
//			else if(finpathStack[(k-1)*PE+2] == paths[finpathStack[k*PE]].rightJunction){
//				// Search for this path connection, if already there, count up
//				struct pathEdge* pathEdge = paths[finpathStack[k*PE]].rightPath;
//				struct pathEdge* lastpathEdge;
//				while(pathEdge){
//					if(pathEdge->ID == finpathStack[l*PE] && pathEdge->depth == k-l){
//						// Path already existent -> count up
...
//					pathEdge->counter = 1;
//					pathEdge->depth = k-l;
//					if(!paths[finpathStack[k*PE]].rightPath){
//						pathEdge->next = paths[finpathStack[k*PE]].rightPath;
//						paths[finpathStack[k*PE]].rightPath = pathEdge;
//					}
//					else{
//						pathEdge->next = NULL;
//						lastpathEdge->next = pathEdge;
...
//			}
//			else{
//				printf("2 Both junctions are not registered as end junctions of this path\nAbort\n");
//				exit(1);
//			}
//		}
//	}
